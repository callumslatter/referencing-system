...
let arrayOfRefsForFormatting: number[] = [];

// Keep track of the instance being worked on
declare var refInstanceCount: number;
refInstanceCount = 0;

let lastRefToBeFormatted: number;

// let remainingText: string = "";
let finalText: string = "";

// REGEX
const REFERENCE_IDENTIFICATION_REGEX: RegExp = /\[(.+?)\]/g;
...
function refFormatter(inputText: string) {
  ...
  for (const rawRef of rawRefs) {
    ...
    remainingText = inputText.slice(
      rawRef.index! + rawRef[0].length + 1
    );
  ...
  const referencesLeftToFormat =
    arrayOfRefsForFormatting.length > 0 ? true : false;
...
function recordExists(rawRef: RegExpMatchArray) {
...
function concatReferences() {
  ...
  const formattedReferences = formatGroupOfReferences(
...
function formatGroupOfReferences(arrayOfRefsForFormatting: number[]) {
  // If less than three references, output references in current state
  if (
    arrayOfRefsForFormatting.length === 1 ||
