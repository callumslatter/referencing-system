...
function referenceExtractor(inputText: string) {
  ...
  referenceLibrary.forEach((reference) => {
    reference.locations?.forEach((location) => {
      const referenceLocation: IReferenceInstances = {
...
function textFormatter(inputText: string) {
  ...
  let groupOfReferences: IReference[] = [];
  ...
  // For each referenceLocation, check if the next reference's index is the same as the current references index plus
  // its length, plus 2. If it is, add it to a group to be formatted together. If it's not, do something with the group to be formatted together
  // then, carry on checking. 
