...
// Declaring regexes
const referenceIdentificationRegex = /\[(.+?)\]/g;
...
function referenceExtractor(inputText: string) {
  ...
  const rawReferences = inputText.match(referenceIdentificationRegex);
  ...
  rawReferences?.forEach((reference) => {
    const debracketedReference = reference.replace(squareBracketRegex, "");
...
const extractedReferences = referenceExtractor(inputText);
